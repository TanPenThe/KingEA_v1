...
- Timestamp: 2026-08-09T00:04:36.5962630+08:00
- Type: Gate 1 pre-execution provenance correction after fail-closed manual-launch attempt
- Status: CORRECTED AND VERIFIED; NO RESULT-BEARING PASS STARTED
- Incident: The first owner-invoked manual batch stopped before MT5 launch with `PRE_TOOLING_VERIFICATION_FAILED:PRE_TOOLING_GATE1_PREP_V3_20260808.json`. The older preparation manifest correctly detected that `tests/Test-Stage14ResearchReadinessPolicy.ps1` had changed after preparation, but the launcher lacked a governed rule for the intentional policy update introduced with the exact-root authorization and manual-batch controls.
- Correction: The launcher now performs one deep provenance verification across the retained preparation manifest and the final manual-batch manifest. It permits only the exact policy path `tests/Test-Stage14ResearchReadinessPolicy.ps1` to supersede its preparation hash, and only when the current policy SHA-256 is explicitly bound by `dependency_hashes.policy_source`. Any other changed, missing, size-mismatched, or unbound source remains a hard failure.
- Source provenance: Corrected final sources are bound by `governance/evidence/stage14/PRE_TOOLING_GATE1_MANUAL_BATCH_V7_20260809.json`; file SHA-256 `47E14908BE3A5975E8C6D419013AF4989D6EC0188DB8D7076E9D3D29F9975D26`; canonical manifest SHA-256 `1A8605DC9A0ADA363206D44C98317A6C8A6D197C265C2AFA11DEF029F4936E3E`. V7 chains to V6 canonical manifest SHA-256 `75455DA9A80F4408D3DC34666692826B749CBDFD88E34FF1B548C35C39FC256C` and preserves the approved root and authorization hashes.
- Verification: The focused seven-test manual-batch suite PASS includes a negative fixture proving that a canonically valid V7 manifest with an incorrect policy binding fails as `POLICY_SUPERSESSION_NOT_BOUND`. Direct read-only reproduction returns `PASS_GOVERNED_POLICY_SUPERSESSION`. All 60 Python tests and all 18 repository PowerShell policy suites PASS.
- Safety state: MT5/MetaTester process count 0 and Gate 1 execution-directory count 0 after the failed attempt and correction. Candidate remains `FROZEN`; candidate-budget consumption remains 0. Approved Gate 1 root `BC4D5D84DBF45AAB6628AA0E1D39D984F715217BB1CA1C092DE1EE97385FA889`, partitions, grid, data, candidate, and owner authorization remain unchanged.
...
- Timestamp: 2026-08-09T09:25:35+08:00
- Type: Gate 1 infrastructure invalidation, shared-input concurrency repair, and replacement-root preparation
- Status: PRIOR CHILD INVALIDATED AND PRESERVED; REPLACEMENT ROOT VERIFIED; AWAITING NEW EXPLICIT OWNER APPROVAL
- Failure evidence: child `G1-0000` under root `BC4D5D84DBF45AAB6628AA0E1D39D984F715217BB1CA1C092DE1EE97385FA889` produced 978 complete result frames, 22 missing configuration frames, and hard-failure frames for configurations 295 and 470. The retained record `governance/evidence/stage14/gate1_execution_20260808/INFRASTRUCTURE_INVALIDATION_G1-0000.json` classifies the child as `INVALIDATED_PRESERVED_NOT_SELECTION_ELIGIBLE`; none of its partial results may be used or reused.
- Root cause: concurrent local tester agents opened the immutable manifest and calendar inputs without `FILE_SHARE_READ`. Twenty-two synchronized authorization-guard refusals account exactly for the 22 missing frames; two additional concurrent-read failures occurred at the end-of-pass authorization recheck and account for both hard-failure frames.
- Repair: `ResearchOpenInputFile` now opens both sandbox and Common-Files inputs with `FILE_SHARE_READ`. The repair is covered by a source-level regression, compiled with 0 errors and 0 warnings, and produces `GuardedResearchTester.ex5` SHA-256 `00BD0FAD6E9829510B0011C44C951735CEE7EA8BC770BBE2F36FA959DA5DC600`.
- Candidate-budget accounting: the first valid result-bearing pass started under the prior root, so Candidate 001's family budget is now consumed exactly once. The replacement root records `candidate_budget_before=1` and `candidate_budget_transition=ALREADY_CONSUMED`; it cannot consume a second attempt.
